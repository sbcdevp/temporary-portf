export default function SplitText(identifier, vars) {
    ...
    function findPos(node) {
      ...
      if (node.offsetParent) {
        do {
          if (
            node.offsetParent &&
            node.offsetParent == document.getElementsByTagName('html')[0]
          ) {
            // needHTML = false;
          }
    ...
    if (vars && typeof vars == 'object' && vars !== null) {
      ...
      if (vars.type && typeof vars.type == 'string') {
        ...
        for (var i = 0; i < vars.type.length; i++) {
          if (
            possible.indexOf(vars.type[i].toLowerCase()) != -1 &&
            use.indexOf(vars.type[i].toLowerCase()) == -1
          ) {
      ...
      this.vars.charsClass =
        vars.charsClass && typeof vars.charsClass == 'string'
          ? vars.charsClass
          : defaults.charsClass
  
      //if wordsClass is set then use it
      this.vars.wordsClass =
        vars.wordsClass && typeof vars.wordsClass == 'string'
          ? vars.wordsClass
          : defaults.wordsClass
  
      //if linesClass is set then use it
      this.vars.linesClass =
        vars.linesClass && typeof vars.linesClass == 'string'
          ? vars.linesClass
          : defaults.linesClass
      ...
      this.vars.position =
        vars.position && allowedPositions.indexOf(vars.position) != -1
          ? vars.position
          : defaults.position
    } else {
    ...
    this.revert = function() {
      for (var i = 0; i < this.HTMLobjects.length; i++) {
        this.savedObjects.push(this.HTMLobjects[i])
    ...
    for (var i = 0; i < this.HTMLobjects.length; i++) {
      ...
      if (this.vars.type.indexOf('lines') != -1) {
        ...
        for (var j = 0; j < splitPoints.length; j++) {
          ...
          if (
            this.vars.linesClass !== undefined &&
            this.vars.linesClass != 'undefined'
          ) {
      ...
      if (this.vars.type.indexOf('words') != -1) {
        function splitWords(parent, st) {
          ...
          parent.innerHTML =
            startTag +
            parent.innerHTML.replaceAll(' ', endTag + ' ' + startTag) +
            endTag
            ...
            if (
              ...
              if (j != nodes.length - 1) {
                newClass = newClass.replaceAll(
                  '**',
                  nodes[j].innerHTML + nodes[j + 1].innerHTML
                )
              } else {
                newClass = newClass.replaceAll('**', '')
              }
      ...
      if (this.vars.position == 'absolute' || this.vars.position == 'fixed') {
        for (var j = currentLists.chars.length - 1; j >= 0; j--) {
          currentLists.chars[j].style.width =
            currentLists.chars[j].offsetWidth + 'px'
          currentLists.chars[j].style.height =
            currentLists.chars[j].offsetHeight + 'px'
          currentLists.chars[j].style.left =
            currentLists.chars[j].toBe.left + 'px'
          currentLists.chars[j].style.top = currentLists.chars[j].toBe.top + 'px'
        }
  
        for (var j = currentLists.words.length - 1; j >= 0; j--) {
          currentLists.words[j].style.width =
            currentLists.words[j].offsetWidth + 'px'
          currentLists.words[j].style.height =
            currentLists.words[j].offsetHeight + 'px'
          currentLists.words[j].style.left =
            currentLists.words[j].toBe.left + 'px'
          currentLists.words[j].style.top = currentLists.words[j].toBe.top + 'px'
        }
  
        for (var j = currentLists.lines.length - 1; j >= 0; j--) {
          currentLists.lines[j].style.width =
            currentLists.lines[j].offsetWidth + 'px'
          currentLists.lines[j].style.height =
            currentLists.lines[j].offsetHeight + 'px'
          currentLists.lines[j].style.left =
            currentLists.lines[j].toBe.left + 'px'
